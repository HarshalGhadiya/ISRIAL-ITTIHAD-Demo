...
const SystemAdminTable = () => {
  ...
  const rSelected = useSelector(
    (state) => state?.root?.adminRoleType?.tabflag
  );
  ...
  useEffect(() => {
    ...
    console.log(userSite, 'userSite', rSelected)
      ...
      <CardHeader className="mb-2 d-flex justify-content-between align-items-center">
        ...
        <div>
          ...
          <Button
            ...
            disabled={loadingexportbutton || (rSelected === 1
              ? systemadminData?.adminData?.length === 0
              : systemadminTypeData?.roleData?.length === 0)}
            color="primary"
            className={`rounded-0 ${
              CombinePermission ? "cursor-not-allowed" : "cursor-pointer"
            }`}
