...
const EditComment = () => {
    ...
    const textTranslate = async (singalCommentData) => {
        ...
        const [convertedText, editedText] = await Promise.all([singalCommentData?.originalComment && translateText(singalCommentData?.originalComment, usersite == "israelBackOffice" ? 'ar' : 'iw'), singalCommentData?.updatedComment && translateText(singalCommentData?.updatedComment, usersite == "israelBackOffice" ? 'ar' : 'iw')])
    ...
    //strip harmfulword
    // const filterHarmfulWords = (input, harmfulWords) => {
    ...
    const stripCss = (input) => {
      ...
      //comment handle change
    const handleInputChange = async (e) => {
        const text = e.target.value;     
          const sanitizedInput = stripEmails(stripUrls(stripJavascript(stripHtml(stripSql(stripCss(text))))))
        setInputText(sanitizedInput);
        // Translate
        const TranslationText = await translateText(sanitizedInput, usersite == "israelBackOffice" ? 'ar' : 'iw');
                                    ...
                                    <Select
                                        ...
                                        styles={{
                                            option: (provided, state) => ({
                                                ...provided,
                                                opacity: state.data.value === jsonData.inActiveValue ? 0.5 : 1,  // Apply opacity if the value matches
                                            }),
                                        }}
                                        value={options.find((option) => option.value === status)}
                                        options={options.map(option => (
                                            option.value === jsonData.inActiveValue
                                                ? { ...option, isDisabled: true }  // Disable the "pending" option
                                                : option
                                        ))}
                                        isClearable={false}
                                        onChange={(e) => {
